...
function TechnicalForm() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            // @ts-ignore
            const { data, isFetching, isError } = await createTechnical(values);
            setIsLoading(false);
            if (isError) {
                toast({
                    variant: "destructive",
                    title: "Technical Event Created error",
                });
                setIsLoading(false);
                return;
            }
            toast({
                title: "Technical Event Created",
            });
            setFormDirty(false);
            form.reset();
            navigate({
            ...
            toast({
                title: "Error while creating technical entry.",
            });
            setIsLoading(false);
        }
    }
