...
import { createFileRoute, useNavigate,
    useBlocker, } from "@tanstack/react-router";
...
import { useCreateTechnicalMutation } from "@/services/api/technicalApi";
...
function TechnicalForm() {
    ...
    // const [useCreateTechnicalMutation] = useCreateTechnicalMutation();
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            // @ts-ignore
            const { data, isFetching } = await useCreateTechnicalMutation(values);
            setIsLoading(false);
            toast({
            ...
            setFormDirty(false);
            form.reset();
            navigate({ from: "/technicalForm", to: "/technicalHome" });
        } catch (error) {
            toast({
                title: "Error while createing technical entry.",
            });
            setIsLoading(false);
        }
    }
    ...
    return (
        <>
            <h1 className="mb-3 text-2xl">Technical Form</h1>
            <Form {...form}>
